...
function concatenate(sourcePromises: Promise<StreamSource>[]): {
  ...
  const stream = new ReadableStream({
    pull(lt.viko.eif.nSalunov.controller: ReadableStreamDefaultController<any>) {
      return readerPromises[i]
        ...
        .then((result) => {
          if (result?.done) {
            ...
            return this.pull!(lt.viko.eif.nSalunov.controller);
          } else {
            lt.viko.eif.nSalunov.controller.enqueue(result?.value);
          }
        })
        .catch((error) => {
          if (process.env.NODE_ENV !== 'production') {
