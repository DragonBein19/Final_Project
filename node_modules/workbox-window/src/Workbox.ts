...
class Workbox extends WorkboxEventTarget {
  ...
  > {
    ...
    this._isUpdate = Boolean(navigator.serviceWorker.lt.viko.eif.nSalunov.controller);
    ...
    // If we have a compatible lt.viko.eif.nSalunov.controller, store the lt.viko.eif.nSalunov.controller as the "own"
    ...
    if (
      ...
      // a compatible lt.viko.eif.nSalunov.controller.
      this._sw = waitingSW;
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (navigator.serviceWorker.lt.viko.eif.nSalunov.controller) {
        if (this._compatibleControllingSW) {
          logger.debug(
  ...
  private _getControllingSWIfCompatible() {
    const lt.viko.eif.nSalunov.controller = navigator.serviceWorker.lt.viko.eif.nSalunov.controller;
    if (
      lt.viko.eif.nSalunov.controller &&
      urlsMatch(lt.viko.eif.nSalunov.controller.scriptURL, this._scriptURL.toString())
    ) {
      return lt.viko.eif.nSalunov.controller;
    } else {
      return undefined;
    }
  }
      ...
      switch (state) {
        ...
        case 'activated':
          if (isExternal) {
            ...
            if (sw !== navigator.serviceWorker.lt.viko.eif.nSalunov.controller) {
              logger.warn(
  ...
  private readonly _onControllerChange = (originalEvent: Event) => {
    const sw = this._sw;
    const isExternal = sw !== navigator.serviceWorker.lt.viko.eif.nSalunov.controller;

    // Unconditionally dispatch the controlling event, with isExternal set
    // to distinguish between lt.viko.eif.nSalunov.controller changes due to the initial registration
 ...
 * of the new [lt.viko.eif.nSalunov.controller]{@link https://developer.mozilla.org/en-US/docs/Web/API/ServiceWorkerContainer/lt.viko.eif.nSalunov.controller}
